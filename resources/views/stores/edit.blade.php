<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('stores.update', $store) }}" autocomplete="off"
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <select name="branch_id" required class="w-full border-slate-300 rounded">
                            @foreach ($branches as $branch)
                                <option value="{{ $branch->id }}" @selected(old('branch_id', $store->branch_id) == $branch->id)>
                                    {{ $branch->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $store->name) }}" required autocomplete="off"
                               class="w-full border-slate-300 rounded">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Store Code</label>
                        <input type="text" name="code" value="{{ old('code', $store->code) }}" required autocomplete="off"
